...
const vertexShader = `
  ...
  void main() {
    ...
    float noise = cnoise(vec3(pos.x * 0.03, pos.y * 0.03, uTime * 0.08));
    pos.z += noise * 10.0; 
    
    // Respiración lenta global del relieve
    pos.z += sin(pos.x * 0.01 + uTime * 0.3) * cos(pos.y * 0.01 + uTime * 0.2) * 4.0;
    
    // EFECTO TÁCTICO: Domo topográfico concéntrico que sigue al mouse
    float dist = distance(pos.xy, uMousePos.xy);
    float mouseDome = smoothstep(100.0, 0.0, dist) * 35.0; // Pico de 35 de altura en el cursor
    pos.z += mouseDome;
    
    // 2. EFECTO "ONDA DE AGUA" (RIPPLE) INTERACTIVO Y ESCALONADO (SNAP DIGITAL)
    float ripplePhase = -dist * 0.4 + uTime * 4.0;
    float stepPhase = floor(ripplePhase * 5.0) / 5.0; // Pasos digitales
    
    float ripple = sin(stepPhase) * cos(dist * 0.15);
    
    // Atenuamos la onda (radio de 50 unidades)
    float rippleDecay = smoothstep(50.0, 0.0, dist);
    
    // Sumamos la onda táctica a la altura del terreno
    pos.z += ripple * rippleDecay * 12.0 * uRippleIntensity;
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Rejilla Táctica de Radar (Surveillance Grid) basada en coordenadas XY
    ...
    float gridVal = smoothstep(0.8, 1.0, gridLine) * 0.25; // Opacidad de la rejilla
    
    // Barrido de escaneo horizontal sutil
