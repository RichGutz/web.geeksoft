...
const vertexShader = `
  ...
  void main() {
    ...
    // MODELADO DE VALLE UNIFICADO: Pendiente parabólica suave con ruido homogéneo continuo
    float distFromCenter = abs(pos.x);
    
    // 1. Pendiente parabólica que esculpe la forma de cañón/valle de manera continua
    float valleySlope = pow(distFromCenter / 110.0, 2.0) * 32.0;
    
    // 2. Ruido perlin continuo aplicado por igual a toda la malla para evitar desintegración central
    float noise = cnoise(vec3(pos.x * 0.025, pos.y * 0.02, uTime * 0.06));
    
    // Sumamos la pendiente del cañón y el ruido uniforme de forma integrada
    pos.z += valleySlope + (noise * 10.0); 
    
    // Respiración lenta global del relieve
    pos.z += sin(pos.x * 0.01 + uTime * 0.3) * cos(pos.y * 0.01 + uTime * 0.2) * 2.0;
    
    // EFECTO TÁCTICO: Domo topográfico concéntrico sutil y elegante que sigue al mouse
    float dist = distance(pos.xy, uMousePos.xy);
    float mouseDome = smoothstep(80.0, 0.0, dist) * 14.0;
    pos.z += mouseDome;
    
    // EFECTO RIPPLE DIGITAL SUTIL
    ...
    // Pasamos la posición Z al fragment shader
    vPosition.z = pos.z;
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Curvas principales (hilos muy finos y definidos)
...
export default function ContourTerrain() {
  ...
  useFrame((state) => {
    ...
    if (mat && mat.uniforms) {
      // 1. Asignar tiempo si existe y tiene value
      if (mat.uniforms.uTime && mat.uniforms.uTime.value !== undefined) {
        mat.uniforms.uTime.value = state.clock.elapsedTime;
      }
      
      // 2. Copiar posición de mouse si existe y tiene value con función copy
      ...
      // 3. Asignar intensidad si existe y tiene value
  ...
  return (
    ...
    >
      {/* Reducimos de 256 a 128 para optimizar vértices en GPU de forma masiva */}
